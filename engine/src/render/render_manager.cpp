...
namespace engine
{
    ...
    render_manager* render_manager::create_instance(const create_info& info)
    {
        ...
        log::log("[render_manager::create_instance] Creating instance of render manager ({})...", info.api);
        s_instance = create_instance_impl(info);
        if (s_instance == nullptr)
        {
            log::fatal("[render_manager::create_instance] Failed to create instance of render manager!");
            return nullptr;
        }
        if (!s_instance->init(info))
        {
            log::fatal("[render_manager::create_instance] Failed to initialize render manager instance!");
            s_instance->clear();
            delete s_instance;
            s_instance = nullptr;
            return nullptr;
        }
        log::info("[render_manager::create_instance] Render manager instance created successfully");
        return s_instance;
    }
